...
class BootAndLiveMigrateServer(utils.NovaScenario,
    ...
    def run(self, image, flavor, block_migration=False, disk_over_commit=False,
        ...
        :param kwargs: Optional additional arguments for server creation
        """
        if "force_delete" in kwargs.keys():
            force = kwargs["force_delete"]
        ...
        self._delete_server(server, force=force)
...
class BootServerAttachCreatedVolumeAndLiveMigrate(utils.NovaScenario,
    ...
    def run(self, image, flavor, size, block_migration=False,
            ...
            create_volume_kwargs=None, force_delete=False,
...
class BootAndAssociateFloatingIp(
        utils.NovaScenario,
        cinder_utils.CinderScenario,
    ...
    def run(self, image, flavor, **kwargs):
        ...
        nets = self.context["tenants"][tenant_id]["networks"]
        for net in nets:
            if net["router:external"]:
                net_id = net["id"]
        floatingip = self._create_floatingip(net_id)
        address = floatingip["floatingip"]["floating_ip_address"]
        self._associate_floating_ip(server, address)
...
class BootServerAssociateAndDissociateFloatingIP(
        utils.NovaScenario,
        neutron_utils.NeutronScenario):
